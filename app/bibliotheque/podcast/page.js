...
const podcasts = [
  { title: "Les fondements de la croyance", category: "Aqida", episodes: 12 },
  { title: "Vie des Sahaba, épisode par épisode", category: "Sahaba", episodes: 24 },
  { title: "Comprendre le fiqh du quotidien", category: "Fiqh", episodes: 9 },
];

export default function PodcastPage() {
  const [activeCategory, setActiveCategory] = useState("Tous");
  const filtered =
    activeCategory === "Tous" ? podcasts : podcasts.filter((p) => p.category === activeCategory);

  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <Link href="/bibliotheque" style={styles.back}>← Bibliothèque</Link>
        <div style={styles.title}>Podcasts</div>
      </div>
            ...
            <div key={i} style={styles.item}>
              <div style={styles.thumb}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#B8935A" strokeWidth="2">
                  <path d="M12 1a4 4 0 00-4 4v6a4 4 0 008 0V5a4 4 0 00-4-4zM19 11a7 7 0 01-14 0M12 18v4" />
                </svg>
              </div>
              <div style={styles.mid}>
                <div style={styles.itemTitle}>{p.title}</div>
                <div style={styles.itemMeta}>{p.category} · {p.episodes} épisodes</div>
              </div>
            </div>
          ))}
          {filtered.length === 0 && <p style={styles.empty}>Aucun podcast pour cette catégorie pour l&apos;instant.</p>}
        </div>
      </div>
    </div>
  );
